...
module.exports =  {
  ...
  compile: function(options, callback) {
    ...
    // Search for ```eslint``` configuration
    lintrcPath = findup('.eslintrc', {
      cwd: options.cwd,
      nocase: true
    });

    if (!lintrcPath) {
      // Set linter to ```jshint```.
      config.linter = 'jshint';
      // Search for ```jshint``` configuration
      lintrcPath = findup('.jshintrc', {
        cwd: options.cwd,
        nocase: true
      });
    }
    ...
    if (lintrcPath) {
      ...
      lintConfig = JSON.parse(jsonminify(fs.readFileSync(lintrcPath, 'utf8')));
    }
    ...
    if (config.lint) {
      logger.debug(chalk.cyan('Linter: ' + config.linter));
    }
    // Loading the config to the builder.
    builder.load(config, lintConfig);
